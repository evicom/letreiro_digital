...
- **Android (Chrome):** abra o link publicado → toque no botão flutuante ⬇️. Se o Chrome já tiver liberado o prompt nativo, ele instala direto; senão o botão mostra o passo a passo (menu ⋮ → "Instalar app / Adicionar à tela inicial").
- **iPhone/iPad (Safari):** o iOS **nunca** mostra o prompt automático de instalação — toque no botão ⬇️ para ver o passo a passo (Compartilhar → "Adicionar à Tela de Início").
- **Computador (Chrome/Edge):** clique no ícone de instalação da barra de endereço, ou no botão ⬇️ dentro do app.
- **Firefox:** não instala PWAs no computador; no celular use "Adicionar à tela inicial" pelo menu.

> ⚠️ O botão de instalação **só funciona de verdade em HTTPS** (GitHub Pages já serve assim). Abrindo o `index.html` direto do computador (`file://`) o navegador nunca oferece a instalação — para testar isso localmente, sirva a pasta com um servidor (veja "Testar localmente" abaixo) ou publique no GitHub Pages.

## Manter a tela sempre ativa

O botão 💡 ativa o modo "não apagar a tela" (Screen Wake Lock API, com um fallback automático para navegadores mais antigos). Toque novamente para desativar. Isso é útil para deixar o letreiro rodando continuamente sem o aparelho bloquear a tela sozinho.
